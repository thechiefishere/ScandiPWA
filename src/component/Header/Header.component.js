...
export const CartOverlay = lazy(() =>
  import(
    /* webpackMode: "lazy", webpackChunkName: "cart" */ 'Component/CartOverlay'
  )
);
export const MyAccountOverlay = lazy(() =>
  import(
    /* webpackMode: "lazy", webpackChunkName: "account" */ 'Component/MyAccountOverlay'
  )
);
...
export class Header extends NavigationAbstract {
  ...
  renderAccountOverlayFallback() {
    return <PopupSuspense actualOverlayKey={CUSTOMER_ACCOUNT_OVERLAY_KEY} />;
  }
  ...
  renderMinicart(isVisible = false) {
    const { onMinicartOutsideClick, isCheckout, device } = this.props;
  ...
  renderContacts() {
    const { header_content: { contacts_cms } = {} } =
