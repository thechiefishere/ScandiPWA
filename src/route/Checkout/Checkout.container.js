...
export const mapDispatchToProps = (dispatch) => ({
  ...
  setNavigationState: (stateName) =>
    dispatch(changeNavigationState(BOTTOM_NAVIGATION_TYPE, stateName)),
  ...
  checkEmailAvailability: (email) =>
    CheckoutDispatcher.then(({ default: dispatcher }) =>
...
export class CheckoutContainer extends PureComponent {
  ...
  onShippingEstimationFieldsChange(address) {
    ...
    fetchMutation(
      CheckoutQuery.getEstimateShippingCosts(address, guestQuoteId)
    ).then(
  ...
  _getCheckoutTotals() {
    ...
    const {
      paymentTotals: { shipping_amount },
    } = this.state;

    return shipping_amount ? { ...cartTotals, shipping_amount } : cartTotals;
  }
  ...
  saveGuestEmail() {
    ...
    const mutation = CheckoutQuery.getSaveGuestEmailMutation(
      email,
      guestCartId
    );
  ...
  prepareAddressInformation(addressInformation) {
    const {
      shipping_address: { id, save_in_address_book, ...shippingAddress } = {},
      billing_address: {
        id: dropId,
  ...
  trimAddressMagentoStyle(address) {
    ...
    if (region_id) {
      ...
      const { available_regions } =
        countries.find(({ id }) => id === country_id) || {};
      ...
      const { code } =
        available_regions.find(({ id }) => +id === +region_id) || {};
  ...
  async saveBillingAddress(paymentInformation) {
    ...
    const { shippingAddress: { id: shippingAddressId = null } = {} } =
      this.state;
    ...
    await fetchMutation(
      CheckoutQuery.getSetBillingAddressOnCart({
        guest_cart_id,
        same_as_shipping,
        billing_address: billingAddress,
      })
  ...
  async savePaymentMethodAndPlaceOrder(paymentInformation) {
    ...
    try {
      ...
      const orderData = await fetchMutation(
        CheckoutQuery.getPlaceOrderMutation(guest_cart_id)
      );
      const {
        placeOrder: {
          order: { order_id },
